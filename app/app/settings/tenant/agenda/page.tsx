...
export default async function TiposDeAgendamentoPage() {
  ...
  const [{ data: tipos }, { data: pessoas }, { data: org }] = await Promise.all([
    supabase
      ...
      .select(
        "id, name, slug, description, category, duration_minutes, location_kind, location_details, default_owner_user_id, requires_confirmation, is_active, reminder_enabled, reminder_minutes_before, reminder_extra_offsets_minutes, reminder_body",
      )
