...
beforeEach(() => {
  ...
  state.message = {
    ...
    metadata: { source: "waha" }, revoked_at: null,
  };
});
...
describe("ocultar mensagem recebida no CRM", () => {
  it("preserva o conteúdo e marca a mensagem como oculta", async () => {
    ...
    expect(state.calls[0]).toEqual({ table: "messages", update: {
      metadata: expect.objectContaining({ source: "waha", crm_hidden_by: "usuario-1", crm_hidden_at: expect.any(String) }),
    } });
  ...
  it("restaura a visibilidade sem perder os metadados originais", async () => {
    state.message = { ...state.message, metadata: { source: "waha", crm_hidden_at: "ontem", crm_hidden_by: "usuario-1" } };
    const res = await DELETE(new NextRequest(url, { method: "DELETE" }), ctx);
    expect(res.status).toBe(200);
    expect(state.calls[0]).toEqual({ table: "messages", update: { metadata: { source: "waha" } } });
