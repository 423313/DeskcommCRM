...
API de instalação e admissão exige plataforma, sessão MFA válida e ausência de acompanhamento de suporte. Ativação/configuração exige `requireRole("admin")`, com guarda de suporte para escrita. Neste perfil, configurar exige membership administrativo real e sair do acompanhamento de suporte; não é criada uma autoridade delegada nova por callback. Leitura/uso exige viewer. Concessão `navigation.tasks` só abre a porta já existente; não cria autoridade sobre tarefas e não dá leitura delas ao pacote.
