-- 0263 — Documentos declarativos locais; nenhuma execução de pacote ou DDL dinâmico.
...
create table if not exists public.extension_operations (
  id uuid primary key,
  kind text not null check (kind in ('catalog_admission','install','configure')),
  status text not null check (status in ('preparing','completed','failed','cancelled')),
  ...
  constraint extension_operations_preparing check (status <> 'preparing' or kind = 'install')
);
...
begin
  ...
  if v_catalog.id is null then
    ...
    update public.extension_operations set status='cancelled',error_code='extension_catalog_stale',updated_at=now()
      where catalog_id=v_catalog.id and kind='install' and status='preparing';
  end if;
...
create or replace function public.fn_extensions_prepare_install(p_actor uuid, p_operation uuid, p_catalog uuid, p_publisher text, p_name text, p_version text)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare
  v_request jsonb := jsonb_build_object('kind','install','actor',p_actor,'catalog',p_catalog,'publisher',p_publisher,'name',p_name,'version',p_version);
  v_op public.extension_operations; v_catalog public.extension_catalogs; v_entry jsonb;
  v_install public.extension_installations; v_sha text;
begin
  perform public.fn_extensions_assert_actor(p_actor);
  if p_operation is null or p_catalog is null or p_publisher is null or p_name is null or p_version is null then
    raise exception using errcode='P0001',message='extension_invalid_input';
  end if;
  perform pg_advisory_xact_lock(255,1);
  perform public.fn_extensions_assert_actor(p_actor);
  ...
  if exists (select 1 from public.system_update_runs where status='dispatched') then
  ...
  select * into v_install from public.extension_installations where catalog_id=p_catalog and publisher=p_publisher and name=p_name;
  if found then
    if v_install.version <> p_version then raise exception using errcode='P0001',message='extension_version_update_unsupported'; end if;
    select sha256 into v_sha from public.extension_artifacts where id=v_install.artifact_id;
    if v_sha <> v_entry->>'sha256' then raise exception using errcode='P0001',message='extension_version_conflict'; end if;
  end if;
  if v_install.id is null then
    if exists (select 1 from public.extension_operations where kind='install' and status='preparing'
    ...
    if (select count(*) from public.extension_installations) +
  ...
  insert into public.extension_operations(id,kind,status,actor_id,catalog_id,installation_id,publisher,name,version,
    ...
    values(p_operation,'install',case when v_install.id is null then 'preparing' else 'completed' end,
      p_actor,p_catalog,v_install.id,p_publisher,p_name,p_version,v_request,public.fn_extensions_fingerprint(v_request),
      v_catalog.revision,v_catalog.digest,v_entry,
      case when v_install.id is null then null else jsonb_build_object('installation',to_jsonb(v_install)) end)
    returning * into v_op;
  return to_jsonb(v_op);
end $$;
...
declare
  ...
  v_artifact public.extension_artifacts; v_install public.extension_installations; v_sha text; v_document_json jsonb;
begin
  perform public.fn_extensions_assert_actor(p_actor);
  perform pg_advisory_xact_lock(255,1);
  perform public.fn_extensions_assert_actor(p_actor);
  ...
  if v_op.kind <> 'install' or v_op.actor_id is distinct from p_actor then
  ...
  if v_op.status in ('cancelled','failed') then return to_jsonb(v_op); end if;
  ...
  if v_op.status='completed' then
    select a.* into v_artifact from public.extension_artifacts a join public.extension_installations i on i.artifact_id=a.id where i.id=v_op.installation_id;
    if v_artifact.manifest is distinct from p_manifest or v_artifact.document is distinct from p_document then raise exception using errcode='P0001',message='extension_artifact_mismatch'; end if;
    return to_jsonb(v_op);
  end if;
  if exists (select 1 from public.system_update_runs where status='dispatched') then
    raise exception using errcode='P0001',message='extension_core_update_in_progress';
  ...
  select * into v_install from public.extension_installations where catalog_id=v_op.catalog_id and publisher=v_op.publisher and name=v_op.name;
  if found then
    if v_install.version <> v_op.version then raise exception using errcode='P0001',message='extension_version_update_unsupported'; end if;
    select sha256 into v_sha from public.extension_artifacts where id=v_install.artifact_id;
    if v_sha <> p_sha256 then raise exception using errcode='P0001',message='extension_version_conflict'; end if;
  end if;
  ...
  update public.extension_operations set status='completed',installation_id=v_install.id,
    result=jsonb_build_object('installation',to_jsonb(v_install)),updated_at=now() where id=p_operation returning * into v_op;
  return to_jsonb(v_op);
end $$;

create or replace function public.fn_extensions_fail_install(p_actor uuid, p_operation uuid, p_error_code text)
returns jsonb language plpgsql security definer set search_path = public, pg_temp as $$
declare v_op public.extension_operations;
begin
  perform public.fn_extensions_assert_actor(p_actor);
...
begin
  ...
  select * into v_op from public.extension_operations where id=p_operation for update;
  if not found then raise exception using errcode='P0001',message='extension_operation_not_found'; end if;
  if v_op.kind <> 'install' then raise exception using errcode='P0001',message='extension_operation_conflict'; end if;
...
begin
  ...
  perform public.fn_extensions_assert_actor(p_actor,p_organization);
  select a.manifest into v_manifest from public.extension_installations i join public.extension_artifacts a on a.id=i.artifact_id
    where i.id=p_installation;
  ...
  insert into public.organization_extensions(organization_id,installation_id,enabled,configuration,revision,updated_by)
    ...
    on conflict (organization_id,installation_id) do update set enabled=excluded.enabled,configuration=excluded.configuration,
      revision=excluded.revision,updated_by=excluded.updated_by,updated_at=now() returning * into v_link;
...
begin
  if new.status='dispatched' then
    perform pg_advisory_xact_lock(255,1);
    if exists (select 1 from public.extension_operations where kind='install' and status='preparing') then
      raise exception using errcode='P0001',message='extension_preparation_in_progress';
    end if;
  end if;
  return new;
...
revoke execute on function public.fn_extensions_prepare_install(uuid,uuid,uuid,text,text,text) from public,anon,authenticated;
...
grant execute on function public.fn_extensions_prepare_install(uuid,uuid,uuid,text,text,text) to service_role;
