...
- Audit é append-only, e isso é do SCHEMA e não da prosa: nenhum papel tem GRANT de UPDATE, DELETE **nem TRUNCATE** em `api_audit_log` — **nem `service_role`**. Para conferir na fonte em vez de acreditar nesta linha:
  ...
  **`TRUNCATE` entra na consulta de propósito, e desde a migration 0255 o
  ...
  pela REST (o PostgREST não emite `TRUNCATE`), então não era buraco de
  superfície; era o único privilégio concedido capaz de apagar auditoria, e o
  pior deles — não passa por RLS (RLS filtra linha, e ele não olha linha), não
  passa pelas policies (que só existem para INSERT e SELECT) e não deixa rastro,
  porque não sobra tabela.

  **A lição que sobrevive ao conserto é sobre a sonda, não sobre o grant.** Uma
  sonda que pergunte só por `DELETE`/`UPDATE` devolve zero linhas e deixa quem
  leu concluindo que a tabela não pode ser esvaziada, quando o privilégio que a
  esvazia INTEIRA está lá — e foi exatamente o que
  `tests/invariants/retencao-poda-e-expurgo.test.ts` mediu por meses, verde, com
  o TRUNCATE concedido ao lado. Hoje ele pergunta pelos três, e tem controle
  positivo de que INSERT e SELECT continuam de pé: `revoke` largo demais mataria
  a auditoria deixando o caso verde.
