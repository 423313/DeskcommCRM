 ...
 *
 * Quem grava `cliente` é SQL (o trigger e o backfill da migration 0252); quem
...
const MIGRATION = join(
  process.cwd(),
  "supabase/migrations/20260915120000_0252_cliente_nasce_do_agendamento.sql",
);
...
describe("a etiqueta de cliente", () => {
  it("é a mesma no TypeScript e no SQL que a escreve", () => {
    ...
    expect(
      ocorrencias.length,
      `a migration 0252 tem de escrever '${TAG_DE_CLIENTE}' no trigger E no backfill`,
