 ...
 * NOVO sobre banco que ainda não tem a 0252 (clone que não atualizou, `db push`
...
async function linhaDoBanco(): Promise<LinhaDoApp | null> {
  try {
    const { data, error } = await createAdminClient()
      ...
      .eq("id", 1)
      .maybeSingle();
    // Clone que ainda não aplicou a 0252 devolve 42P01 aqui. Isso NÃO é erro
 ...
 * ao ambiente, e depois da 0252 isso diria "não dá para receber pelo canal
